...
function calculateNetSalary(salary, profit){
    ...
    const deductTax = calculateTaxRate(monthlyTaxablePay)// %

    const grossPay = salary - ((deductTax / 100) * salary)

    const deductNhif = calculateDeduction(grossPay)

    const pay = salary - ((deductTax / 100) * salary) - deductNhif

    const deductNssf = calculatePensionablePayTier(pay)

    const netSalaryCalculation = () =>{var netSalaryValue = (pay - deductNhif) ; return netSalaryValue}
 ...
 const salary = parseInt("7000")// gross salsary

 const profit = parseInt("800")// profit
